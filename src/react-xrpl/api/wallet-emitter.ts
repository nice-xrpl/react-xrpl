import { Client as xrplClient, TransactionStream, Wallet as xrplWallet, dropsToXrp, getNFTokenID } from 'xrpl';
import { EventEmitter } from 'events'
import { isIssuedCurrency } from 'xrpl/dist/npm/models/transactions/common';
import { isCreatedNode, isModifiedNode, Node } from 'xrpl/dist/npm/models/transactions/metadata';
...
function getBalance(account: string, nodes: Node[]) {
	...
	for (const node of nodes) {
		if (isCreatedNode(node) && node.CreatedNode.LedgerEntryType === 'AccountRoot') {
			if (node.CreatedNode.NewFields.Balance && node.CreatedNode.NewFields.Account && node.CreatedNode.NewFields.Account === account) {
				balance = node.CreatedNode.NewFields.Balance as string;
			}
		}

		if (isModifiedNode(node) && node.ModifiedNode.LedgerEntryType === 'AccountRoot') {
			if (node.ModifiedNode.FinalFields?.Balance && node.ModifiedNode.FinalFields?.Account === account) {
	...
	private onTransaction = (tx: TransactionStream) => {
		if (tx.transaction.TransactionType === 'NFTokenMint') {
			if (tx.transaction.Account === this._address) {
				...
				const balance = getBalance(this._address, tx.meta?.AffectedNodes ?? []);
				this.emit(WalletEvent.BalanceChange, balance, dropsToXrp(balance));
			}
		}
