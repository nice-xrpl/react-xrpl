...
import { Currency, WalletInitialState, XRPLWalletInitialState } from './wallet-types';

export async function createWallet(client: xrplClient, amount: string = '1024'): Promise<XRPLWalletInitialState> {
...
export async function createWalletFromSeed(client: xrplClient, seed: string): Promise<XRPLWalletInitialState> {
	await client.connect();
...
export async function getInitialWalletState(client: xrplClient, address: string): Promise<WalletInitialState> {
	await client.connect();
